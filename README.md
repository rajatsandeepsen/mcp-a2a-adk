# mpc-a2a-adk
- [What is AI Engineer](https://www.latent.space/p/ai-engineer)

![Image](/images/phase-2.png)

## MCP

![Image](/images/phase-3.5.png)

- [MCP Official Website](https://modelcontextprotocol.io)
- [MCP UI - AI Engineer YT](https://youtu.be/o-zkvb0iFDQ)
- [MCP UI Guide](https://mcpui.dev/guide/server/typescript/overview)

## A2A Protocol

![Image](/images/phase-5.png)

- [Agent2Agent Protocol - A2A by Google](https://a2a-protocol.org)
- [AI Agent Protocols Explained: MCP, A2A, ACP and More](https://youtu.be/tb0c70DROYE)

## ADK

- [Agent Development Kit - ADK by Google](https://adk.dev/)
- [How to build an AI agent with MCP, ADK, and A2A on Google Cloud](https://youtu.be/6mQwHqK1I5w)
- [ADK Example](https://github.com/google/adk-samples/blob/main/python/agents/currency-agent/README.md)

## Other Protocols

- [Agent Communication Protocol - ACP by IBM](https://agentcommunicationprotocol.dev)
- [Agent Network Protocol - ANP](https://github.com/agent-network-protocol/AgentNetworkProtocol)

## Commercial Protocols

- [Machine Payments Protocol - MPP by Stripe](https://mpp.dev/)
- [Universal Commerce Protocol - UCP by Stripe](https://ucp.dev/)
- [Agent Auth Protocol - AAP by Better-Auth](https://agentauthprotocol.com/)

## SDKs

- [CrewAI](https://docs.crewai.com/)
- [LlamaIndex](https://developers.llamaindex.ai)
- [Langchain](https://docs.langchain.com/)
- [Vercel's AI SDK (ts)](https://ai-sdk.dev/)
- [Python AI SDK](https://pythonaisdk.mintlify.app)
